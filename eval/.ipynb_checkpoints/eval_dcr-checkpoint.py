...
def save_dcr_score(file_path, score, dcrs_real, dcrs_test, csv_file):
    """
    Uses the directory of the script as the base path, appends data_name,
    ensures the directory exists, then writes the score to DCR_score.txt.

    :param data_name: Subdirectory name to be created under the script's location.
    :param score: Score value to write into the file.
    """
    ...
    df = {}
    df['syn_to_real'] =  dcrs_real
    df['syn_to_test'] =  dcrs_test
    # Convert to DataFrame and save
    df = pd.DataFrame(df)  # Wrap in list to make it one row
    df.to_csv(csv_file, index=False)
    print(f'DCR_ditributions has been saved on {csv_file}')
...
if __name__ == '__main__':
    ...
    os.makedirs(dir_path, exist_ok=True)  # create the directory, not the file path
    os.makedirs(csv_path, exist_ok=True)  # create the directory
    save_dcr_score(file_path, score, dcrs_real.flatten().tolist(), dcrs_test.flatten().tolist(), csv_file = csv_path + f'/{model}.csv')
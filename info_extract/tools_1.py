...
#             # print(self.columns, self.cat_columns, self.num_columns)
...
#         # value_to_find = 'MDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMDMD'
#         # locations = df_train_encoded[df_train_encoded == value_to_find].stack().index.tolist()
...
def compute_correlation_divergence(real_data, synthetic_data, show_numbers=True, title=None, vmin=0, vmax=1, save_path=None):
    """
    ...
    Returns:
    pd.DataFrame: Divergence matrix.
    """
    ...
    def ensure_categorical(df, reference_df):
    ...
    ax = sns.heatmap(divergence, annot=show_numbers, cmap="Greens", cbar=True, 
...
def total_variation_distance(real_col, synthetic_col):
    """Computes the Total Variation Distance (TVD) for categorical data."""
    # Convert categorical values to integer codes
    real_codes = real_col.cat.codes
    synthetic_codes = synthetic_col.cat.codes

    # Ensure both distributions cover the same range
    minlength = max(real_codes.max(), synthetic_codes.max()) + 1

    # Compute normalized frequency distributions
    real_dist = np.bincount(real_codes, minlength=minlength) / len(real_codes)
    synthetic_dist = np.bincount(synthetic_codes, minlength=minlength) / len(synthetic_codes)

    return 0.5 * np.sum(np.abs(real_dist - synthetic_dist))
...
def evaluate_column_wise(real_data, synthetic_data, Columns, num_columns):
    ...
    for col in Columns:
        if col in num_columns:
            ...
            real_col = real_data[col].astype('category')

            synthetic_col = synthetic_data[col].astype(pd.CategoricalDtype(categories=real_col.cat.categories))

            results[col] = total_variation_distance(real_col,
                                                    synthetic_col)
    return results
...
def _extract_columns_(json_path):
    # Check if JSON file exists
    if json_path is not None and os.path.exists(json_path):
        ...
        if "cat_col_idx" in data and "column_names" in data:
            ...
            cat_columns = [data["column_names"][idx] for idx in data["cat_col_idx"]]
        
        # Extract num column indices and names
        if "num_col_idx" in data and "column_names" in data:
            num_columns = [data["column_names"][idx] for idx in data["num_col_idx"]]
        
        if data["task_type"] != "regression":
            cat_columns.extend([data["column_names"][idx] for idx in data["target_col_idx"]])
        elif data["task_type"] == "regression":
            num_columns.extend([data["column_names"][idx] for idx in data["target_col_idx"]])
        else: 
            ttt = data["task_type"]
            raise ValueError(f"task_type = {ttt} is unknown")
    ...
    return columns, cat_columns, num_columns
...
def update_csv(file_path, data_name, row_indices, values):
    # Define the index and corresponding values
    index_values = {
     row_indices[i]: values[i] for i in range(len(row_indices))
    }

    # Check if the file exists
    if os.path.exists(file_path):
        df = pd.read_csv(file_path, index_col=0)  # Read existing file
    else:
        df = pd.DataFrame(index=index_values.keys())  # Create new DataFrame

    # Ensure the column exists, adding it if necessary
    if data_name not in df.columns:
        df[data_name] = None

    # Update values
    for row, value in index_values.items():
        df.loc[row, data_name] = value

...
# try:
#     import diskannpy  # Make sure DiskANN is installed
#     diskann_available = True
# except ImportError:
#     diskann_available = False

def compute_min_distances(A, B, k=1, indexing='KDTree', timing=False, search_depth=64, n_links_per_node=16):
    """
    Computes the minimum distances from each point in set A to its nearest neighbour(s) in set B.

    Parameters:
    -----------
    A : np.ndarray
        A NumPy array of shape (N, D), representing N query points in D-dimensional space.
    
    B : np.ndarray
        A NumPy array of shape (M, D), representing M reference points in D-dimensional space.
    
    k : int, optional, default=1
        The number of nearest neighbours to find for each query point in A.
    
    indexing : str, optional, default='KDTree'
        The method used for nearest neighbour search. Options:
        - 'KDTree' : Uses SciPy's KDTree (efficient for low-dimensional data).
        - 'faiss' : Uses FAISS exact nearest neighbour search (fast for high-dimensional data).
        - 'faiss_hnsw' : Uses FAISS HNSW for approximate nearest neighbour search (best for large datasets).
        - 'diskann' : Uses DiskANN (CPU-based approximate nearest neighbour search, optimized for large-scale datasets).
    
    timing : bool, optional, default=False
        If True, prints the execution time of the nearest neighbour search.
    
    search_depth : int, optional, default=64 (Only for 'faiss_hnsw' and 'diskann')
        Controls the search depth for FAISS HNSW and DiskANN. Higher values improve accuracy but slow down queries.
    
    n_links_per_node : int, optional, default=32 (Only for 'faiss_hnsw')
        Controls the number of links per node in FAISS HNSW. Higher values improve accuracy but increase memory usage.

    Returns:
    --------
    distances : np.ndarray
        A NumPy array of shape (N, k), containing the distances to the k-nearest neighbours for each point in A.
    ...
    N, D = 1000000, 128  # 1M points in 128D space
    A = np.random.rand(N, D).astype('float32')
    B = np.random.rand(N, D).astype('float32')

    # Using FAISS exact search
    distances = compute_min_distances(A, B, k=1, indexing='faiss', timing=True)

    # Using FAISS HNSW (approximate, much faster)
    distances_hnsw = compute_min_distances(A, B, k=1, indexing='faiss_hnsw', search_depth=100, timing=True)

    # Using DiskANN (optimized for large-scale datasets)
    distances_diskann = compute_min_distances(A, B, k=1, indexing='diskann', search_depth=100, timing=True)
    ```
    """
    start_time = time.time() if timing else None  # Start timing if enabled

    if indexing == 'KDTree':
        # Build a KDTree for dataset B
        tree = KDTree(B)
        
        # Query the tree with each point in A to find the k-nearest neighbours
        distances, _ = tree.query(A, k)

    elif indexing == 'faiss':
        d = A.shape[1]
        index = faiss.IndexFlatL2(d)  # L2 (Euclidean) distance index
        index.add(B)  # Add dataset B to FAISS index
        D, I = index.search(A, k)  # Find nearest neighbours
        distances = np.sqrt(D)  # Convert squared L2 distances to Euclidean distances

    elif indexing == 'faiss_hnsw':  
        d = A.shape[1]
        m = n_links_per_node  # Number of links per node in HNSW (higher = more accurate but slower)
        index = faiss.IndexHNSWFlat(d, m)  # HNSW for fast approximate search
        index.hnsw.efSearch = search_depth  # Adjust search depth (higher = more accurate)
        
        index.add(B)  # Add dataset B to FAISS index
        D, I = index.search(A, k)  # Find nearest neighbours
        distances = np.sqrt(D)  # Convert squared L2 distances to Euclidean distances

    elif indexing == 'diskann':

        d = A.shape[1]

        # Create a DiskANN index
        index = diskannpy.build_memory_index(B, metric="l2", complexity=search_depth)
        
        # Perform the nearest neighbour search
        distances, _ = diskannpy.search_memory_index(index, A, k)

    else:
        raise ValueError("Invalid indexing method. Choose 'KDTree', 'faiss', 'faiss_hnsw', or 'diskann'.")

    # Calculate and print timing if enabled
    if timing:
        end_time = time.time()
        print(f"{indexing} search time: {end_time - start_time:.4f} seconds")
    ...
    if partition_portion == 'leave_one_out':
        ...
        for _ in range(n):
            ...
            if mode == 'percentile':
                ...
                p = (percentile * m)//100
                d_held_out_to_train = compute_min_distances(held_out, train, k = p, indexing = indexing)
                ...
                d_held_out_to_train = compute_min_distances(held_out, train, k = 1, indexing = indexing)
...
def DCR_dist_comparison(real_data, list_synt_data, evaluating_metric, names = None, title = None):
    # Estimate KDE for real data
    ...
    # Color palettes
    # fill_colors = ['rgba(0, 0, 255, 0.2)', 'rgba(0, 255, 0, 0.2)', 'rgba(255, 165, 0, 0.2)', 'rgba(128, 0, 128, 0.2)']
    ...
    fill_colors = [
        ...
        'rgba(154, 205, 50, 0.2)',    # Yellow Green
        'rgba(255, 105, 180, 0.2)',   # Hot Pink
        'rgba(105, 105, 105, 0.2)',   # Dim Grey
        'rgba(0, 191, 255, 0.2)',     # Deep Sky Blue
        'rgba(160, 32, 240, 0.2)',    # Purple (alt)
        'rgba(210, 105, 30, 0.2)',    # Chocolate
        'rgba(124, 252, 0, 0.2)',     # Lawn Green
    ...
    # Update layout
    fig.update_layout(
        title="Density Comparison"  if title is None else title,
        xaxis_title="Value",
        yaxis_title="Density",

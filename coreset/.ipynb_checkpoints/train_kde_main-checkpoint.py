...
def run_kde(args):
    ...
    for t in [0, 1, 100, 500, 999]:
        print(f"t={t}, sigma_t={get_sigma_t(t, T=1000, scheduler='cosine'):.4f}")
    ...
    # coreset_points_init = torch.tensor(real_copula_encoded[indices], dtype=torch_dtype)
    ...
    loss_path = os.path.join(loss_dir, f"{args.data_name}_{args.n_samples}_{args.coreset_size}.csv")
    ...
    output_file = f'coreset_{args.bandwidth}_{args.coreset_size}.csv'
    synth.to_csv(os.path.join(save_path, output_file), index=False)
    cprint(f"Synthetic data saved to {output_file}", color='green')

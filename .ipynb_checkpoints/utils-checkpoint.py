...
def execute_function(method, mode):
    print(f'BUILDING  execute_function with method = {method} and mode = {mode}')
    if method == 'vae':
        mode = 'train'
    mode = 'main' if mode == 'train' else 'sample'

    if method == 'vae':
        module_name = f"tabkde.vae.main"
    elif method == 'tabsyn':
        module_name = f"tabkde.{mode}"
    elif method == 'tabddpm':
        module_name = f"baselines.tabddpm.main_train" if mode == 'main' else f"baselines.tabddpm.main_sample"
    elif method in ['KDE_VAE_encoding', 'simple_KDE_VAE_encoding']:
        module_name = f"tabkde.{mode}"
        print(f'module_name is {module_name}')
    elif method == 'copula_diff_encoding':
        module_name = f"tabkde.copula_diff_encoding.{mode}"
    elif method in ['simple_KDE', 'TabKDE']:
        if mode == 'main':
            module_name = f"tabkde.copula_encoding.{mode}"
        else:
            module_name = f"tabkde.{mode}"
        print(f'module_name to load is {module_name}')
    elif method == 'pass':
        mode = 'sample'
        module_name = f"tabkde.{mode}"
        print(f'module_name to load is {module_name}')
    ...
    try:
        ...
        print(f'The model: {module_name} has been successfully loaded')

    except ModuleNotFoundError:
        # Print all directories Python is searching for modules
        print(f'Python is searching for the model: {module_name} in the following directories:')
...
def get_args():
    ...
    parser.add_argument('--method', type=str, default='tabkde', help='Method: tabkde or baseline.')
    ...
    # configs for Num Epochs in traing TabSyn's VAE and Difussion models
    parser.add_argument('--num_epochs', type=int, default= 4000, help='Num Epochs.')
    ...
    parser.add_argument('--decorrelation', type=bool, default=False, help='decorrelation?')
    parser.add_argument('--cat_encoding_ordinal', type=str, default='ours', help='ordinal cat encoding?')
    parser.add_argument('--copula_encoder', type= int, default = 1, help='copula_encoder?')
    parser.add_argument('--latent_encoding', type=str, default = 'vae', help='what latent encoding to use?')
    parser.add_argument('--rank_encoding', type=int, default = 1, help='Use ranking in cat encoding?')

...
def main(args):
    ...
    df_train_encoded, _ = ordinal_encoder_model.fit(train_data, decorrelation = decorrelation, 
                                                    json_path = info_path, cat_encoding = cat_encoding)
    
    save_model(ordinal_encoder_model, name = 'data_aware_model', path = ckpt_dir)
    print(f'ordinal_encoder_model has been saved in {ckpt_dir}')
    ...
    if copula_encoder:
        ...
        print(f'copula_model has been saved in {ckpt_dir}')
     
    np.save(f'{ckpt_dir}/train_z.npy', df_train_encoded.to_numpy())
    #for IBM or large data we need to subsample .....
    data_name_ = info['name']
    print(f'staring to learn DCR dist for {data_name_}.')

    gauss_model, _, _ = fit_mix_gaussian_to_(df_train_encoded, partition_portion=0.5, 
                                             max_number_components=10, 
                                             saved_path_for_plot=f'{ckpt_dir}/mix_gaussian.png', 
                                                   random_selection_size = 10000 if data_name_ == 'ibm_func' else None,
                                                   down_sampling = 50000 if data_name_ == 'ibm_func' else None,
                                            )

    
    
    save_model(gauss_model, name = 'gauss_model', path = ckpt_dir)
    print(f'mix_gauss_model for DCR dist has been saved on {ckpt_dir}.')

    print(f"Successfully saved pretrained embeddings (mix_gaussian_model, ordinal_encoder_model {'and copula_model' if copula_encoder else ''}) on disk!")  

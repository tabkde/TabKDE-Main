...
from tabkde.vae.model import Decoder_model 
import joblib
...
def convert_sample_z_to_df(sample_z, args):
    dataname = args.dataname
    curr_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = f'{curr_dir}/{args.latent_encoding}/ckpt/{dataname}/'
    initial_encoder_model = load_model(name = 'data_aware_model', path = model_path)
    if not args.copula_encoder:
       print('Why args.copula_encoder is False???????')
    copula_model = load_model(name = 'copula_model', path = model_path)
    synth_data_encoded = copula_model.convert(sample_z)

    synth_data = initial_encoder_model.decode(synth_data_encoded)
    return synth_data


def get_input_train(args):
    dataname = args.dataname
    if args.method in ['simple_KDE', 'TabKDE'] and args.latent_encoding != 'copula_encoding':
       print(f'Since args.method is {args.method}, args.latent_encoding should be copula_encoding but it is {args.latent_encoding}')
       raise ValueError(f"Ivalid latent_encoding")
    ...
    if args.latent_encoding == 'vae':
    ...
    elif  args.latent_encoding in ['copula_diff_encoding', 'copula_encoding']: 
        curr_dir = os.path.dirname(os.path.abspath(__file__))
        dataset_dir = f'data/{dataname}'
        ...
        ckpt_dir = f'{curr_dir}/{args.latent_encoding}/ckpt/{dataname}/'
        embedding_save_path = f'{curr_dir}/{args.latent_encoding}/ckpt/{dataname}/train_z.npy'
        print(embedding_save_path)
        train_z = torch.tensor(np.load(embedding_save_path)).float()
    
        B, in_dim = train_z.size()
        train_z = train_z.view(B, in_dim)
        if args.method == 'TabKDE' or args.method == 'simple_KDE':
             dataname = args.dataname
             curr_dir = os.path.dirname(os.path.abspath(__file__))
             model_path = f'{curr_dir}/{args.latent_encoding}/ckpt/{dataname}/'
             info['mix_gaussian'] = load_model('gauss_model', model_path)
             print('mix_gaussian has been loaded in info[mix_gaussian]')
...
def split_num_cat_target(syn_data, info, num_inverse, cat_inverse, device):
    ...
    if info['task_type'] == 'regression':
        ...
        print('syn_cat.shape =', syn_cat.shape)

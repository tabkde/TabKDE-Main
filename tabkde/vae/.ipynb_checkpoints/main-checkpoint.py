...
from tabkde.vae.model import Model_VAE, Encoder_model, Decoder_model
...
def main(args):
    ...
    for epoch in range(num_epochs):
        ...
        num_batches = 0
        with torch.no_grad():
            for batch_num_test, batch_cat_test in test_loader:
                batch_num_test = batch_num_test.to(device)
                batch_cat_test = batch_cat_test.to(device)
                Recon_X_num, Recon_X_cat, mu_z, std_z = model(batch_num_test, batch_cat_test)
        
                mse_loss, ce_loss, kl_loss, acc = compute_loss(batch_num_test, batch_cat_test, Recon_X_num, Recon_X_cat, mu_z, std_z)
        ...
        val_loss = val_mse_loss * 0 + val_ce_loss  # Adjust if needed
        ...
        #     val_mse_loss, val_ce_loss, val_kl_loss, val_acc = compute_loss(X_test_num, X_test_cat, 
        #                                                                    Recon_X_num, Recon_X_cat, mu_z, std_z)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambd', type=float, default=0.7, help='Decay of Beta.')
     # configs for Num Epochs in traing TabSyn's VAE 
